...
const Botones = () => {
  // Toast social (opcional, si quieres feedback)
  const [showSocialToast, setShowSocialToast] = React.useState(null);

  const handleSocial = (provider) => {
    setShowSocialToast({ type: 'info', message: `Funcionalidad de ${provider} en desarrollo` });
  };
  ...
  return (
    <>
      ...
      <div className="social-buttons">
        <button className="social-btn google-btn" type="button" onClick={() => handleSocial('Google')}>
        ...
        <button className="social-btn facebook-btn" type="button" onClick={() => handleSocial('Facebook')}>
          <svg width="20" height="20" viewBox="0 0 24 24">
